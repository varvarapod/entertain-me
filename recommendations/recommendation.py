...
def get_data():
    movie_data = pd.read_csv('../data/imdb_mini.csv')
...
def combine_data(data):
    data_recommend = data.drop(columns=['movie_name', 'imdb_id', 'title', 'plot', 'poster_url', 'year',
                                        'rating', 'votes', 'recomendations'])
    data_recommend['combine'] = data_recommend['genres']
    data_recommend = data_recommend.drop(columns=['genres'])
    return data_recommend
...
def recommend_movies(title, data, combine, transform):
    ...
    movie_id = data['imdb_id'].iloc[movie_indices]
    movie_title = data['title'].iloc[movie_indices]
    movie_genres = data['genres'].iloc[movie_indices]

    recommendation_data = pd.DataFrame(columns=['IMDB_Id', 'Title', 'Genres'])

    recommendation_data['IMDB_Id'] = movie_id
